...
### 단일 서버 비용 절충안 (RAM 32GB → 16GB)
- **사양 조정**: Standard G2, vCPU 4 · RAM 16GB, SSD 256GB (필요시 200GB)
- **예상 비용**: 서버 요금 약 170,240원 + 스토리지 256GB 약 29,000원 → 월 **~200,000원**
- **운영 전제**
  1. Celery 워커 동시성(concurrency)을 2~3개로 제한하고, LLM/ETL 배치 작업은 야간에 몰아서 실행
  2. Redis 메모리 사용량을 1GB 이하로 제한하고 TTL을 짧게 설정
  3. 정적 자산/로그는 빌드 후 Object Storage로 이동해 SSD 사용량을 줄임
  4. CPU 피크가 잦으면 vCPU 6 또는 8로 소폭 상향 고려 (RAM 16GB 유지)
- **장점**: 월 비용을 기존 8vCPU·32GB 대비 약 20만 원 절감, 전체 예산을 40만 원대 초반으로 끌어내림
- **주의점**: 동시 작업이 많은 경우 CPU 경합이 발생할 수 있으므로 워커 수 조절과 모니터링이 필수, 추후 부하 증가 시 24GB 이상으로 확장 검토
